...
def export_clusters_to_folders(labels, dataset, output_folder, max_images_per_cluster=None):
    print(f"\n--> Đang xuất ảnh ra '{output_folder}' "
          f"(tối đa {max_images_per_cluster} ảnh/cụm)...")
    ...
    print(f"--> HOÀN TẤT! Đã lưu {len(clusters)} cụm vào '{output_folder}'.")
...
def plot_visualization(features, labels, n_clusters, adjacency_matrix,
    ...
    plt.close()
    print(f"--> HOÀN TẤT! Đã lưu '{filename}'.")
...
def run_baseline(features_data, true_labels, dataset):
    ...
    print(f"  n_neighbors={BASELINE_N_NEIGHBORS} | threshold={BASELINE_THRESHOLD} | K={BASELINE_K}")
    print(f"  Score formula: {ALPHA:.1f} * ARI + {1-ALPHA:.1f} * NMI")
    print("="*80)

    t_graph    = time.time()
    sim_matrix = build_similarity_matrix(features_data, BASELINE_N_NEIGHBORS, BASELINE_THRESHOLD)
    print(f"--> Graph xong: {time.time()-t_graph:.2f}s | edges={sim_matrix.nnz}")
    ...
    print("\n[BASELINE RESULT]")
    print(f"  {'n_neighbors':<20} : {BASELINE_N_NEIGHBORS}")
    print(f"  {'threshold':<20} : {BASELINE_THRESHOLD}")
    print(f"  {'n_clusters':<20} : {BASELINE_K}")
    print(f"  {'assign_labels':<20} : cluster_qr")
    print(f"  {'affinity':<20} : precomputed (k-NN, symmetrized)")
    print(f"  {'n_edges':<20} : {sim_matrix.nnz}")
    print(f"  {'clustering_time':<20} : {elapsed:.2f}s")
    print(f"  {'ARI':<20} : {ari:.4f}")
    print(f"  {'NMI':<20} : {nmi:.4f}")
    print(f"  {'Score (α={:.1f})':<20} : {score:.4f}".format(ALPHA))
    print("="*80)

    print_top5_clusters(labels, true_labels)

    np.save('spectral_baseline_labels.npy', labels)
    print("--> Đã lưu nhãn vào 'spectral_baseline_labels.npy'")
...
def run_tuning(features_data, true_labels, dataset):
    ...
    print(f"  n_neighbors   : {TUNING_N_NEIGHBORS_LIST}")
    print(f"  threshold     : {TUNING_THRESHOLD_LIST}")
    print(f"  K (n_clusters): {TUNING_K_LIST}")
    print(f"  Score formula : {ALPHA:.1f} * ARI + {1-ALPHA:.1f} * NMI")
    print(f"  Tổng số lần chạy SC: {total_runs}")
    print("="*80)
    ...
    for n_nbrs in TUNING_N_NEIGHBORS_LIST:
        for thr in TUNING_THRESHOLD_LIST:
            print(f"\n--> Xây graph: n_neighbors={n_nbrs}, threshold={thr}...")
            t_graph    = time.time()
            sim_matrix = build_similarity_matrix(features_data, n_nbrs, thr)
            print(f"    Xong: {time.time()-t_graph:.2f}s | edges={sim_matrix.nnz}")

            for k in TUNING_K_LIST:
                run_count += 1
                print(f"  [{run_count:>2}/{total_runs}] K={k}...", end=" ", flush=True)
                ...
                print(f"ARI={ari:.4f} | NMI={nmi:.4f} | "
                      f"Score={score:.4f} (α={ALPHA:.1f}) | {elapsed:.1f}s")

                # So sánh bằng score tổng hợp thay vì chỉ ARI
                if score > best["score"]:
                    best = result

    # Bảng tổng hợp, sắp xếp theo score giảm dần
    print("\n" + "="*90)
    ...
    print(f"\n[BEST TUNED CONFIGURATION]  (tiêu chí: Score = {ALPHA:.1f}×ARI + {1-ALPHA:.1f}×NMI)")
    print(f"  {'n_neighbors':<20} : {best['n_neighbors']}")
    print(f"  {'threshold':<20} : {best['threshold']}")
    print(f"  {'n_clusters':<20} : {best['k']}")
    print(f"  {'assign_labels':<20} : cluster_qr")
    print(f"  {'affinity':<20} : precomputed (k-NN, symmetrized)")
    print(f"  {'n_edges':<20} : {best['n_edges']}")
    print(f"  {'ARI':<20} : {best['ari']:.4f}")
    print(f"  {'NMI':<20} : {best['nmi']:.4f}")
    print(f"  {'Score (α={:.1f})':<20} : {best['score']:.4f}".format(ALPHA))
    print("="*90)

    print_top5_clusters(best['labels'], true_labels)

    np.save('spectral_tuned_labels.npy', best['labels'])
    print("--> Đã lưu nhãn tốt nhất vào 'spectral_tuned_labels.npy'")
...
def main():
    # 1. Nạp features
    try:
        features_data = np.load('features_no_umap.npy')
        print(f"--> Đã nạp features: shape={features_data.shape}, dtype={features_data.dtype}")
    ...
    if len(true_labels) != len(features_data):
        print("Lỗi: Số nhãn không khớp với số features.")
        return

    # 3. Chạy 2 phase
    baseline_result = run_baseline(features_data, true_labels, dataset)
    tuned_result    = run_tuning(features_data, true_labels, dataset)
